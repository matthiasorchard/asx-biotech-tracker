...
async function getRiskMatrixData() {
  ...
  const [catalystsRes, dashboardRes] = await Promise.all([
    supabase
      .from('catalyst')
      .select('ticker, title, event_type, expected_date, impact, confidence')
      .eq('status', 'upcoming')
      .gte('expected_date', today)
      .order('expected_date', { ascending: true }),
    supabase
      .from('company_dashboard')
      .select('ticker, runway_months, cash_at_end, is_cf_positive'),
  ])

  const catalysts = catalystsRes.data ?? []
  const dashboardMap: Record<string, any> = {}
  for (const row of dashboardRes.data ?? []) {
    dashboardMap[row.ticker] = row
  }
  ...
  for (const row of catalysts) {
    const cd = dashboardMap[row.ticker]
    ...
    const runway_label = isCfPos || runway >= 999 ? 'CF+' : `${runway}mo`
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'High impact (≤90d)', value: highImpact.length, color: 'text-green-400' },
